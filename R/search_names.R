...
#' @return A data frame of results, or named list of GUIDs if \code{guids_only} is TRUE. The results should include one entry (i.e. one data.frame row or one list element) per input name. The columns in the data.frame output may vary depending on the results returned by the server, but should include searchTerm, name, rank, and guid.
#' 
#' @examples
#' \dontrun{
#' la_search_names(c("Diatoma tenuis", "Diatoma hyemale var. quadratum", "Macropus"))
#'
#' la_search_names(c("Diatoma tenuis", "Diatoma hyemale var. quadratum", "Macropus"),
#'    guids_only = TRUE)
#'
#' la_search_names("Diatoma tenuis", vernacular = FALSE)
#'
#' la_search_names("Diatoma tenuis", vernacular = TRUE)
#'
#' ## occurrence counts for matched names
#' la_search_names(c("Diatoma tenuis", "Diatoma hyemale var. quadratum",
...
#' }
#' @export
la_search_names <- function(taxa = c(), vernacular = FALSE, guids_only = FALSE, occurrence_count = FALSE, output_format = "simple") {
    ALA4R::search_names(taxa = taxa, vernacular = vernacular, guids_only = guids_only, occurrence_count = occurrence_count, output_format = output_format)
}
