...
#' @param layers string vector: ids of layers to be intersected. The list of possible layers is available from \code{la_fields("layers")}. Names can be passed as full layer names (e.g. "Radiation - lowest period (Bio22)") rather than id ("el871"). Note: if more than one location has been provided in \code{pnts}, the number of layers must be less than 700. 
...
#' @param verbose logical: show additional progress information? [default is set by \code{\link{la_config}}]
#' @return A data.frame or SpatialPointsDataFrame containing the intersecting data information. Missing data or incorrectly identified layer id values will result in NA data
...
#' @export
la_intersect_points <- function(pnts, layers, SPdata.frame = FALSE, use_layer_names = TRUE, verbose = la_config()$verbose) {
 ALA4R::intersect_points(pnts, layers, SPdata.frame, use_layer_names, verbose)
}
