...
#' These are of the form "INDEXEDFIELD:VALUE" e.g. "kingdom:Fungi". See \code{la_fields("general",as_is=TRUE)} for all the 
...
#' fields will be returned. See \code{la_fields("general",as_is=TRUE)} for valid field names. Use \code{fields="all"} to include all available fields
...
#' @seealso \code{\link{la_fields}}, \code{\link{la_config}}
#' @examples
#' \dontrun{
#' ## simplest usage
#' x <- la_taxinfo("rk_genus:Macropus")
#' 
#' ## Data for Fabaceae with specified fields
#' x <- la_taxinfo("rk_family:Fabaceae",fields=c("guid","parentGuid","rk_kingdom","rk_phylum",
#'     "rk_class","rk_order","rk_family","rk_genus","scientificName"))
#' }
#' @export
la_taxinfo <- function(query, fq, fields, verbose = la_config()$verbose, use_data_table = TRUE) {
    ALA4R::taxinfo_download(query = query, fq = fq, fields = fields, verbose = verbose, use_data_table = use_data_table)
}
