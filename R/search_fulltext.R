...
#' See \code{la_fields("general")} for all the fields that are queryable. 
...
#' @param sort_by string: (optional) field to sort on. See \code{la_fields("general")} for valid field names
#' @param sort_dir string: (optional) sort direction, either "asc" or "desc"
#' @seealso \code{\link{la_fields}}
...
#' @examples
#' \dontrun{
#'  ## find information on the red kangaroo
#'  la_config(installation = "au")
#'  la_search_fulltext("Macropus rufus")
#'
#'  ## find genus names like "Oenanthe"
#'  la_search_fulltext("oenanthe", sort_by = "rk_kingdom", fq = "rank:genus")
#' }
#' @export
la_search_fulltext <- function(query, fq, output_format = "simple", start, page_size, sort_by, sort_dir) {
    ALA4R::search_fulltext(query = query, fq = fq, output_format = output_format, start = start, page_size = page_size, sort_by = sort_by, sort_dir = sort_dir)
}
