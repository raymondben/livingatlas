...
# @param caching string: caching behaviour, by default from la_config()$caching
...
# @details Depending on the value of caching, the page is either retrieved from the cache or from the url, and stored in the cache if appropriate. The user-agent string is set according to la_config()$user_agent. The returned response (if not from cached file) is also passed to check_status_code().
#
cached_get <- function(url, type = "text", caching = la_config()$caching, verbose = la_config()$verbose, on_redirect = NULL, on_client_error = NULL, on_server_error = NULL, encoding = la_config()$text_encoding) {
